    ...
    def __getitem__(self,idx):
        ...
        for i in range(self.nc):
            ...
            if self.cpi == 1:
                ...
                if len(img_name.split('_'))==4:
                    img_path = os.path.join(self.root_dir, obj_id, 'obj', img_name)
                    this_x = float(img_name[:-4].split('_')[2])
                    this_y = float(img_name[:-4].split('_')[3])
                    traj.append([this_x,this_y])
                else:
                    img_path = os.path.join(self.root_dir, obj_id, 'other', img_name)
...
if __name__ == '__main__':
    ...
    root_dir_SAD = os.path.join(Path(__file__).parent.parent.parent, 'Data/SimpleAvoid/')
