...
def save_SAD_data(save_path, ts, sim_time):
    ...
    for i in range(sim_time):
        ...
        if i>=500: #random.randint(0, 10)>5:
            ref_path = get_ref_path(1)
        else:
            ref_path = get_ref_path(2)
        start = ref_path[0]
...
save_path = os.path.join(Path(__file__).parent.parent.parent, 'Data/SimpleAvoid2m1cBig/') # save in folder
past = 3
T_range = (10, 10)
ts = 0.2
sim_time = 1000 # [second] or times
